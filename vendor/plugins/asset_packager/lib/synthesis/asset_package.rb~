module Synthesis
  class AssetPackage
    ...
    class << self
      ...
      def find_by_type(asset_type, params = {})
        @@asset_packages_yml[asset_type].map { |p| self.new(asset_type, p, params) }
      end

      def find_by_target(asset_type, target, params = {})
        package_hash = @@asset_packages_yml[asset_type].find {|p| p.keys.first == target }
        package_hash ? self.new(asset_type, package_hash, params) : nil
      end

      def find_by_source(asset_type, source, params = {})
      ...
      def targets_from_sources(asset_type, sources, params = {})
        package_names = Array.new
        sources.each do |source|
          package = find_by_target(asset_type, source, params) || find_by_source(asset_type, source, params)
          package_names << (package ? package.current_file : source)
        end
        package_names.uniq
      end

      def sources_from_targets(asset_type, targets, params = {})
        source_names = Array.new
        targets.each do |target|
          package = find_by_target(asset_type, target, params)
    ...
    def initialize(asset_type, package_hash, params = {})
      ...
      @file_name = "#{append_params(@target, params)}_packaged.#{@extension}"
    ...
    def append_params(name, params)
      params.empty? ? name : "#{name}_#{params.values.join("_")}"
    end
